...
import { evaluateAlbumCompletion } from '../lib/domain/completion';
...
describe('the combine workflow', async () => {
  it('lets an alias play complete the canonical track without rewriting its provenance', async () => {
    const primary = makeEntity('track', 'canonical-track');
    const alias = makeEntity('track', 'source-track');
    ...
    await seed([], [group], [primary, alias]);
    await insertPlays([
      {
        id: 'ply:spotify:source-track:100',
        entityId: alias.id,
        entityType: 'track',
        at: 100,
        ingestedAt: 101,
    ...
    const aggregated = await playsForEntitiesBetween([primary.id], 0, 200);
    const result = evaluateAlbumCompletion({
      tracks: {
        ...
        knownTotal: 1,
        confidence: 'complete',
        excluded: { unavailable: 0, duplicate: 0, nonTrack: 0 },
      },
      plays: aggregated,
      windowMs: 1_000,
      newPlayIds: new Set(['ply:spotify:source-track:100']),
      existing: [],
      recompletion: 'off',
      now: 200,
    });

    expect(result.completion?.playIds).toEqual(['ply:spotify:source-track:100']);
    expect(aggregated[0]?.entityId).toBe(primary.id);
    expect((await listPlays())[0]?.entityId).toBe(alias.id);
  });
