<script lang="ts">
  ...
  import DuelStation from '../components/DuelStation.svelte';
...
<div class="sheet">
  <header class="head">
    <h1 class="display">Weigh up</h1>
    {#if comparableTypes.length > 1}
      <label class="field field--inline">
        <span class="apparatus">Comparing</span>
  ...
  {#if pair}
    <DuelStation a={pair.a} b={pair.b} reason={pair.reason} />
  {:else}
    <Empty
      title="Not enough to weigh"
    ...
    >
      {#snippet action()}
        <a class="btn btn--primary" href={href('/queue')}>Go to the queue</a>
      {/snippet}
    </Empty>
  {/if}
  ...
  {#if history.length > 0}
    <section class="record" aria-labelledby="record-head">
      <div class="head">
        ...
        <span class="apparatus">last {history.length}</span>
      </div>
      <ul class="record__rows">
        {#each history as entry (entry.id)}
          ...
          <li>
            ...
            <span class="apparatus">{relative(entry.at)}</span>
            <button
              type="button"
